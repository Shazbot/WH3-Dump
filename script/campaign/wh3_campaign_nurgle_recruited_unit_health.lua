...
-- Listen for plagues being concocted, save a record in to the savegame of this happening
core:add_listener(
	"nurgle_plague_ritual_listener",
	"RitualStartedEvent",
	true,
	function(context)
		local ritual_name = context:ritual():ritual_key();
		if string.find(ritual_name, "wh3_main_ritual_nur_plague") then
			local performing_faction_key = context:performing_faction():name();
			local prev_plagues_started = cm:get_saved_value("plagues_started_" .. performing_faction_key) or 0;
			cm:set_saved_value("plagues_started_" .. performing_faction_key, prev_plagues_started + 1);
		end;
	end,
	true
);
...
core:add_listener(
	...
	function(context)
		...
		if nurgle_units_to_starting_health_bonus_values[unit:unit_key()] then
			...
			local unit_bonus = get_force_bonus_value(force, nurgle_units_to_starting_health_bonus_values[unit:unit_key()])/100
...
cm:add_first_tick_callback_new(
	function()
		...
		for i = 0, faction_list:num_items() - 1 do
			...
			if faction:subculture() == "wh3_main_sc_nur_nurgle" then
