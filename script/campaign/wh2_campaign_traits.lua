...
function GeneralCompletedBattle(context)
	...
	if side == "Attacker" then
		for i = 1, cm:pending_battle_cache_num_defenders() do
			...
			Give_Traits_For_Fighting_Subcultures(character, faction_name)
		end
	elseif side == "Defender" then
...
function Give_Traits_For_Fighting_Subcultures(character, faction_name)
	...
	if character:faction():subculture() ~= enemy_subculture and SUBCULTURES_TRAIT_KEYS[enemy_subculture] ~= nil then
		if character:won_battle() == true then
			Give_Trait(character, "wh2_main_trait_wins_against_"..SUBCULTURES_TRAIT_KEYS[enemy_subculture]);
		else
			Give_Trait(character, "wh2_main_trait_defeats_against_"..SUBCULTURES_TRAIT_KEYS[enemy_subculture]);
		end
		
		if SUBCULTURES_TRAIT_KEYS[enemy_subculture] == "skaven" then
			RemoveSkavenFromCharacterAndForce(character);
		end
	elseif enemy_culture == "wh2_main_rogue" then
		Give_Trait(character, "wh2_main_trait_wins_against_rogue_armies");
	end
end
	...
	if character:has_region() then
		...
		for i = 1, #corruption_to_trait do
			...
			if current_corruption >= 8 then
				Give_Trait(character, corruption_to_trait[i][2]);
