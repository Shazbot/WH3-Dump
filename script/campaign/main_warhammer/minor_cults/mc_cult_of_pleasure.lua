local minor_cult = {
	key = "",
	slot_key = "wh3_main_slot_set_minor_cult_38",
...
function minor_cult:creation_event(region_key, turn_number)
	...
	cm:change_corruption_in_province_by(region:province_name(), "wh3_main_corruption_slaanesh", 10, "events");
end
			...
			if current_region:is_null_interface() == false and current_region:is_abandoned() == false then
				...
				if owner:is_null_interface() == false and owner:is_human() == true and owner:is_factions_turn() == true then
					if self.valid_subcultures[owner:subculture()] ~= nil then
						...
						local slaanesh_corruption = cm:get_corruption_value_in_region(current_region, "wh3_main_corruption_slaanesh");

						if slaanesh_corruption > 70 then
							valid = false;
						end
