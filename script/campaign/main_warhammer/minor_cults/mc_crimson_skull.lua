local minor_cult = {
	key = "",
	slot_key = "wh3_main_slot_set_minor_cult_37",
...
function minor_cult:creation_event(region_key, turn_number)
	...
	local khorne_corruption = cm:get_corruption_value_in_region(region, "wh3_main_corruption_khorne");

	if khorne_corruption ~= 30 then
		local change = 30 - khorne_corruption;
		cm:change_corruption_in_province_by(region:province_name(), "wh3_main_corruption_khorne", change, "events");
	end
end

function minor_cult:custom_listeners()
	core:add_listener(
		"FactionLeaderDeclaresWar_"..self.key,
		...
		function(context)
			...
			if region and context:character():faction():name() == region:owning_faction():name() then
				cm:change_corruption_in_province_by(region:province_name(), "wh3_main_corruption_khorne", -999, "events");
			end
		end,
		true
	);
...
function minor_cult:custom_event(faction, region, cult_faction)
	...
	if khorne_corruption >= 100 then
		local war_amount = cm:get_factions_bonus_value(faction, "khornes_gift_random_war_count");

		for i = 1, war_amount do
			cult_declare_random_war(faction:name());
		end
			...
			if current_region:is_null_interface() == false and current_region:is_abandoned() == false then
				...
				if owner:is_null_interface() == false and owner:is_human() == true and owner:is_factions_turn() == true then
					if self.valid_subcultures[owner:subculture()] ~= nil then
						...
						local khorne_corruption = cm:get_corruption_value_in_region(current_region, "wh3_main_corruption_khorne");

						if khorne_corruption >= 50 then
							valid = false;
						else
							-- This Cult is pretty brutal, so we need a good justification for why Khorne curses you
							local faction_leader = owner:faction_leader();
							local trait1 = faction_leader:has_trait("wh3_main_trait_defeated_skarbrand");
							local trait2 = faction_leader:has_trait("wh3_dlc26_trait_defeated_arbaal");
							local trait3 = faction_leader:has_trait("wh3_dlc26_trait_defeated_skulltaker");
							local trait4 = faction_leader:has_trait("wh3_dlc20_trait_defeated_valkia");
							
							if trait1 == false and trait2 == false and trait3 == false and trait4 == false then
								valid = false; -- You've not angered Khorne yet
							end
						end
