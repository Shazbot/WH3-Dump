local minor_cult = {
	key = "",
	slot_key = "wh3_main_slot_set_minor_cult_29",
	...
	event_data = {event_chance_per_turn = 10, event_cooldown = 8, event_limit = 999, event_initial_delay = 3, force_trigger = false},
	saved_data = {status = 0, region_key = "", event_cooldown = 0, event_triggers = 0}
};

local angry_faction_effects = {
	"wh2_dlc09_faction_political_diplomacy_mod_tomb_kings",
	"wh2_main_faction_political_diplomacy_mod_dark_elves",
	"wh2_main_faction_political_diplomacy_mod_high_elves",
	"wh2_main_faction_political_diplomacy_mod_lizardmen",
	"wh3_main_faction_political_diplomacy_mod_cathay",
	"wh3_main_faction_political_diplomacy_mod_kislev",
	"wh3_main_faction_political_diplomacy_mod_ogre_kingdoms",
	"wh_dlc05_faction_political_diplomacy_mod_bretonnia",
	"wh_dlc05_faction_political_diplomacy_mod_wood_elves",
	"wh_main_faction_political_diplomacy_mod_dwarfs",
	"wh_main_faction_political_diplomacy_mod_empire",
	"wh_main_faction_political_diplomacy_mod_greenskins",
	"wh_main_faction_political_diplomacy_mod_norsca",
	"wh_main_faction_political_diplomacy_mod_vampire_counts"
};

function minor_cult:custom_event(faction, region, cult_faction)
	angry_faction_effects = cm:random_sort(angry_faction_effects);

	local incident_builder = cm:create_incident_builder("wh3_main_minor_cult_sartosan_vault");
	local payload = cm:create_new_custom_effect_bundle("wh3_main_payload_stolen_loot");
	payload:add_effect(angry_faction_effects[1], "faction_to_faction_own_unseen", -100);
	
	local loot = cm:random_number(50) * 100;

	if cm:model():random_percent(10) then
		-- Extra luck!
		loot = loot + 6000;
		payload:set_duration(7);
	else
		loot = loot + 2000;
		payload:set_duration(4);
	end

	local payload_builder = cm:create_payload();
	payload_builder:treasury_adjustment(loot);
