local minor_cult = {
	key = "",
	slot_key = "wh3_main_slot_set_minor_cult_21",
...
function minor_cult:character_in_region(character, region, slot_list)
	...
	local building_key = "wh3_main_minor_cult_carnival_of_chaos_1";
	...
	if has_building == true then
		...
		if cm:model():random_percent(chance) then
			if character:faction():subculture() ~= "wh3_main_sc_nur_nurgle" then
				campaign_traits:give_trait(character, "wh3_main_trait_carnival_of_chaos", 1, 100);
			end
		end
	end
end
