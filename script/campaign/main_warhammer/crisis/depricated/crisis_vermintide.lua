...
function crisis_vermintide:trigger()
	local potential_skaven = {}
	for faction_key, _ in pairs(self.skaven_factions) do
		local faction = cm:get_faction(faction_key)
		if crisis:check_faction_is_valid(faction, true) then
	...
	if #potential_skaven > 0 then
		for i = 1, #potential_skaven do
			...
			crisis:create_scenario_force(faction_key, region_key, self.army_template, self.unit_list, false, math.floor(self.initial_army_count*crisis.settings.difficulty_mod))
			cm:apply_effect_bundle("wh3_main_ie_scripted_endgame_vermintide", faction_key, 0)
			if crisis.settings.crisis_diplomacy_enabled == false then
		...
		cm:set_saved_value("crisis_vermintide_under_empire_expanding", data)
		cm:set_saved_value("crisis_vermintide_ubersreik_marker_trigger", cm:turn_number()+2)
		cm:set_saved_value("crisis_vermintide_ubersreik_marker", true);
		cm:set_saved_value("crisis_vermintide_ikit_warpfuel", true);
		self:expand_under_empire()
		self:add_listeners()
		if crisis.ultimate_mission_delay < self.ultimate_mission_delay then
			crisis.ultimate_mission_delay = self.ultimate_mission_delay
		end
...
function crisis_vermintide:final_invasion()
	...
	if #potential_skaven > 0 then
		for i = 1, #potential_skaven do
			...
			if region_key ~= nil then
				...
				crisis:create_scenario_force(faction_key, region_key, self.army_template, self.unit_list, true, math.floor(self.base_army_count*endgame.settings.difficulty_mod))
				...
				crisis:no_peace_no_confederation_only_war(faction_key)
				cm:force_change_cai_faction_personality(faction_key, self.ai_personality)
				local region = cm:get_region(region_key)
				crisis:declare_war_on_adjacent_region_owners(faction, region)
			end
		end
		...
		crisis:reveal_regions(region_list)
		cm:activate_music_trigger("ScriptedEvent_Negative", "wh2_main_sc_skv_skaven")

		local incident_key = "wh3_main_ie_incident_endgame_vermintide_2"
		for i = 1, #human_factions do
			crisis:add_victory_condition_listener(human_factions[i], incident_key, objectives)
...
function crisis_vermintide:add_listeners()
	local data = cm:get_saved_value("crisis_vermintide_under_empire_expanding")

	if data then
		core:add_listener(
			"crisis_vermintide_under_empire_expanding",
			"WorldStartRound",
			true,
			function()
				if cm:turn_number() >= data.last_turn then
					self:final_invasion()
					crisis_vermintide.repeat_regions = {}
					cm:set_saved_value("crisis_vermintide_repeat_regions", false)
					crisis_vermintide.under_empire_regions = {}
					cm:set_saved_value("crisis_vermintide_under_empire_regions", false)
					cm:set_saved_value("crisis_vermintide_under_empire_expanding", false)
					core:remove_listener("crisis_vermintide_under_empire_expanding")
				else
					self:expand_under_empire()
				end
			end,
	...
	if cm:get_saved_value("crisis_vermintide_ubersreik_marker") then
		core:add_listener(
			"crisis_vermintide_area_entered",
			"AreaEntered",
			function(context)
			...
			function(context)
				...
				if not character:is_null_interface() and character:has_military_force() then
					...
					if faction:is_human() and (faction:subculture() == "wh_main_sc_emp_empire" or faction:subculture() == "wh_dlc05_sc_wef_wood_elves" or faction:subculture() == "wh_main_sc_dwf_dwarfs") then
						cm:set_saved_value("crisis_vermintide_ubersreik_invasion", true);
						self:generate_ubersreik_battle_force(character)
						crisis_vermintide:ubersreik_battle_cleanup()
					end
				end
			end,
			true
	...
	if cm:get_saved_value("crisis_vermintide_ubersreik_marker_trigger") then
		core:add_listener(
			"crisis_vermintide_add_marker",
			"WorldStartRound",
			function()
				return cm:turn_number() == cm:get_saved_value("crisis_vermintide_ubersreik_marker_trigger")
			end,
			function()
				self:add_marker()
				cm:set_saved_value("crisis_vermintide_ubersreik_marker_trigger", false)
				core:remove_listener("crisis_vermintide_add_marker")
			end,
			true
		)
	end

	if cm:set_saved_value("crisis_vermintide_ikit_warpfuel", true) then
		core:add_listener(
			"crisis_vermintide_ikit_warpfuel",
			"WorldStartRound",
			true,
			function()
...
function crisis_vermintide:expand_under_empire()
	local potential_skaven = {}
	for faction_key, _ in pairs(self.skaven_factions) do
		local faction = cm:get_faction(faction_key)
		if crisis:check_faction_is_valid(faction, false) then
			table.insert(potential_skaven, faction_key)
		end
	end
	...
	cm:set_saved_value("crisis_vermintide_repeat_regions", self.repeat_regions)
	cm:set_saved_value("crisis_vermintide_under_empire_regions", self.under_empire_regions)
end

function crisis_vermintide:expand_under_empire_adjacent_region_check(sneaky_skaven, region, checked_regions)
		...
		if checked_regions[adjacent_region_key] == nil then
			...
			if self.under_empire_regions[sneaky_skaven][adjacent_region_key] == nil and adjacent_region:is_null_interface() == false then
				...
				if dice_roll <= chance then
					out("Crisis vermintide: Spreading under-empire to "..adjacent_region_key.." for "..sneaky_skaven)
...
function crisis_vermintide:add_marker()
	...
	if pos_x > -1 then
		...
		if incident_issued == true then
			...
			cm:set_saved_value("crisis_vermintide_ubersreik_marker", false)
			core:remove_listener("crisis_vermintide_area_entered")
		end
	end

end

function crisis_vermintide:generate_ubersreik_battle_force(character)
	local faction = character:faction()
	...
	local old_invasion = invasion_manager:get_invasion("crisis_vermintide_ubersreik_invasion")
	...
	local invasion = invasion_manager:new_invasion("crisis_vermintide_ubersreik_invasion", "wh2_main_skv_skaven_qb1", units, {character:logical_position_x(), character:logical_position_y()})
	invasion:set_target("CHARACTER", character:command_queue_index(), faction_name)
	invasion:apply_effect("wh_main_bundle_military_upkeep_free_force_crisis", -1)
	invasion:start_invasion(
		function(self)
			core:add_listener(
				"crisis_vermintide_ubersreik_invasion_war_declared",
	...
	core:add_listener(
		"crisis_vermintide_ubersreik_invasion_battle_end_of_round_cleanup",
		"EndOfRound", 
		true,
		function()
			self:kill_ubersreik_invasion();
			cm:set_saved_value("crisis_vermintide_ubersreik_invasion", false)
		end,
		false
	)
end

function crisis_vermintide:ubersreik_battle_cleanup()
	core:add_listener(
		"crisis_vermintide_ubersreik_invasion_battle_cleanup",
		"BattleCompleted",
		function()
			return cm:get_saved_value("crisis_vermintide_ubersreik_invasion")
		end,
		function()
			cm:set_saved_value("crisis_vermintide_ubersreik_invasion", false)
...
function crisis_vermintide:kill_ubersreik_invasion()
	local invasion = invasion_manager:get_invasion("crisis_vermintide_ubersreik_invasion")
...
function crisis_vermintide:generate_ubersreik_army(faction_key)
...
if cm:get_saved_value("crisis_vermintide_ubersreik_invasion") then
	crisis_vermintide:ubersreik_battle_cleanup()
end

if cm:get_saved_value("crisis_vermintide_repeat_regions") then
	crisis_vermintide.repeat_regions = cm:get_saved_value("crisis_vermintide_repeat_regions")
end

if cm:get_saved_value("crisis_vermintide_under_empire_regions") then
	crisis_vermintide.under_empire_regions = cm:get_saved_value("crisis_vermintide_under_empire_regions")
end

crisis_vermintide:add_listeners()