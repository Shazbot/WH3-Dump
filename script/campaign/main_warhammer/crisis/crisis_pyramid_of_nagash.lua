...
function crisis_pyramid_of_nagash:trigger()

	if is_table(episode_nagash_endgame) then
		local nagash_faction_obj = cm:get_faction(episode_nagash_endgame.nagash_faction_key)
		if is_faction(nagash_faction_obj) 
			and nagash_faction_obj:is_null_interface() == false 
			and nagash_faction_obj:is_human() == true
		then
			-- the Nagash faction is human, so don't trigger
			return
		end
	end
	...
	crisis:create_scenario_force(data.faction_key, self.region_key, self.army_template, data.unit_list, true, math.floor(self.base_army_count*crisis.settings.difficulty_mod))
	...
	crisis:no_peace_no_confederation_only_war(data.faction_key)
	local invasion_faction = cm:get_faction(data.faction_key)
	crisis:declare_war_on_adjacent_region_owners(invasion_faction, region)
	table.insert(crisis.revealed_regions, self.region_key)
	...
	for i = 1, #human_factions do
		crisis:add_victory_condition_listener(human_factions[i], data.incident_key, objectives)
	...
	cm:set_saved_value("crisis_pyramid_of_nagash_saved_data", data)
	crisis_pyramid_of_nagash:add_listeners()
end

function crisis_pyramid_of_nagash:add_listeners()
	local data = cm:get_saved_value("crisis_pyramid_of_nagash_saved_data")
	core:add_listener(
		"crisis_pyramid_of_nagash_ultimate_victory",
		"MissionSucceeded",
		function(context)
		...
		function()
			...
			core:remove_listener("crisis_pyramid_of_nagash_spawn_army")
			core:remove_listener("crisis_pyramid_of_nagash_ultimate_victory")
		end,
		true
	)

	core:add_listener(
		"crisis_pyramid_of_nagash_spawn_army",
		"WorldStartRound",
		function()
			return cm:turn_number() % 10 == 0 and cm:get_faction(data.faction_key):has_home_region()
		end,
		function()
			crisis:create_scenario_force(data.faction_key, self.region_key, self.army_template, data.unit_list, false, 1)
		end,
		true
	)
end

if cm:get_saved_value("crisis_pyramid_of_nagash_saved_data") then
	crisis_pyramid_of_nagash:add_listeners()
end