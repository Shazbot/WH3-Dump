...
function crisis_vampires_rise:trigger()
	...
	for faction_key, region_key in pairs(potential_vampires) do
		...
		if not faction:is_human() and not faction:was_confederated() then
			...
			crisis:create_scenario_force(faction_key, region_key, self.army_template, self.unit_list, true, math.floor(self.base_army_count*crisis.settings.difficulty_mod))
