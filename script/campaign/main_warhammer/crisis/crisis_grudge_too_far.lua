...
function crisis_grudge_too_far:trigger()
	...
	for faction_key, region_key in pairs(potential_dwarfs) do
		...
		if not invasion_faction:is_human() and not invasion_faction:was_confederated() then
			...
			if can_be_human then
				army_count = math.floor(self.major_army_count*crisis.settings.difficulty_mod)
			else
				army_count = math.floor(self.minor_army_count*crisis.settings.difficulty_mod)
			end
			crisis:create_scenario_force(faction_key, region_key, self.army_template, self.unit_list, true, army_count)
			if faction_key == "wh_main_dwf_karak_izor" then
				if not invasion_faction:is_dead() and cm:get_region("wh3_main_combi_region_karak_eight_peaks"):owning_faction():name() == faction_key then
					crisis:create_scenario_force(faction_key, "wh3_main_combi_region_karak_eight_peaks", self.army_template, self.unit_list, true, army_count)
				end
			end
			...
			crisis:declare_war_on_adjacent_region_owners(invasion_faction, region, self.subculture)

			cm:apply_effect_bundle("wh3_main_ie_scripted_endgame_grudge_too_far", faction_key, 0)
			table.insert(crisis.revealed_regions, region_key)
		end
	end
