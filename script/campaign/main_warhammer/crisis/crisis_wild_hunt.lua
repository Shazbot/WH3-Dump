...
function crisis_wild_hunt:trigger()
	...
	for faction_key, region_key in pairs(potential_wood_elves) do
		...
		if not faction:is_human() and not faction:was_confederated() then
			...
			crisis:create_scenario_force(faction_key, region_key, self.army_template, self.unit_list, true, 2)
			if faction_key ==  "wh_dlc05_wef_wood_elves" then
				crisis:create_scenario_force(faction_key, "wh3_main_combi_region_the_oak_of_ages", self.army_template, self.unit_list, true, math.floor(self.base_army_count*crisis.settings.difficulty_mod))
			end
			...
			crisis:declare_war_on_adjacent_region_owners(faction, region, self.subculture)
