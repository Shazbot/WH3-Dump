...
crisis_will_of_hashut.portal_regions = {
...
crisis_will_of_hashut.objective_regions = {
...
function crisis_will_of_hashut:trigger()
	...
	for i = 1, #portal_regions do
		...
		crisis:create_scenario_force(data.faction_key, regions, self.army_template, data.unit_list, true, 1)
	end
	...
	cm:set_saved_value("crisis_will_of_hashut_saved_data", data)
	crisis_will_of_hashut:add_listeners()
end

function crisis_will_of_hashut:add_listeners()
	local data = cm:get_saved_value("crisis_will_of_hashut_saved_data")
	local portal_regions = self.portal_regions
	
	core:add_listener(
		"crisis_will_of_hashut_ultimate_victory",
		"MissionSucceeded",
		function(context)
		...
		function()
			...
			core:remove_listener("crisis_will_of_hashut_spawn_army")
			core:remove_listener("crisis_will_of_hashut_ultimate_victory")
			core:remove_listener("crisis_will_of_hashut_force_victory")
		end,
		true
	)
	...
	core:add_listener(
		"crisis_will_of_hashut_spawn_army",
		"WorldStartRound",
		function()
		...
		function()
			if cm:get_faction(self.hashut_data.faction_key):num_generals() <= 24 then
				...
				crisis:create_scenario_force(self.hashut_data.faction_key, spawn_region, self.army_template, data.unit_list, false, 1)
			end
		end,
		true
	)
...
if cm:get_saved_value("crisis_will_of_hashut_saved_data") then
	crisis_will_of_hashut:add_listeners()
end