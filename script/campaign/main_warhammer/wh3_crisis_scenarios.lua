package.path = package.path .. ";data/script/campaign/main_warhammer/crisis/?.lua"
...
crisis = {
	...
	-- Used when the crisis triggers to show points of interest that turn. 
...
-- Initialise the crisis
cm:add_first_tick_callback(
	function()
		if crisis.force_disable_all == true then
			out("Crisis scenarios are disabled - force_disable_all = true")
			return
		end

		-- Get the user settings from the shared state manager and update the default scripted values
		crisis:update_campaign_settings()

		if crisis.settings.crisis_enabled == false then
			out("Crisis scenarios are disabled - crisis features will NOT be activated in this campaign")
			return
		end

		-- Look for crisis scenario files and load them into the framework
		local crisis_scenario_files = core:get_filepaths_from_folder("/script/campaign/main_warhammer/crisis/", "*.lua")
		out("####################")
		out("Crisis scenarios: Loading the following scenarios from /script/campaign/main_warhammer/crisis/:")
		local scenarios_loaded = {}
		local env = core:get_env()

		for i = 1, #crisis_scenario_files do

			local scenario_filepath = crisis_scenario_files[i]
			local scenario_name = tostring(string.sub(scenario_filepath,39,(string.len(scenario_filepath)-4)))

			-- Skip scenarios in the Depricated subfolder
			if not string.find(string.lower(scenario_filepath), "[\\/]depricated[\\/]") then
				...
				if loaded_file then
					...
					if not scenario_executed_successfully then
						...
						-- This condition is a bit unintuitive - The default state of the var is nil as it's not been registered at the time of the check. 
						--However due to scenario_enabled possibility of being false (and in turn, different than nil) we have to explicitly check for whether the var is false/true 
		...
		crisis.scenarios = table.copy(scenarios_loaded)
		if #crisis.scenarios > 0 then
			out(#crisis.scenarios.." total scenarios loaded successfully")
			out("####################")
		else
			out("0 scenarios loaded. This shouldn't happen, but might if a player disables all scenarios in the front end. Crisis feature is DISABLED.")
			out("####################")
			return
		end

		-- Crisis scenarios support both a turn timer and a victory condition for triggering
		-- These listeners are removed after the crisis scenario triggers as we only support the crisis firing once
		if crisis.triggered == false then

			core:add_listener(
				"crisis_turn_trigger_listener",
				"WorldStartRound",
				function()
					return crisis.settings.turn_trigger 
						and cm:turn_number() >= cm:random_number(crisis.settings.turn_max, crisis.settings.turn_min)
						and (is_table(episodes_manager.persistent.active_episodes["end_times"]) == false
						or #episodes_manager.persistent.active_episodes["end_times"] < 1) 
				end,
				function()
					if crisis.triggered == false then
						crisis.triggered = true
						crisis:choose_scenario()
						core:remove_listener("crisis_victory_trigger_listener")
						core:remove_listener("crisis_turn_trigger_listener")
					end
				end,
				true
			)
			...
			core:add_listener(
				"crisis_victory_trigger_listener",
				"MissionSucceeded",
				function(context)
					return crisis.settings.victory_trigger and context:mission():mission_record_key() == crisis.victory_type
				end,
				function()
					if crisis.triggered == false then
						crisis.triggered = true
		...
		if cm:get_saved_value("crisis_scenario_data") then
			crisis:add_early_warning_listener(cm:get_saved_value("crisis_scenario_data"))
		end

		if cm:get_saved_value("crisis_ultimate_crisis_data") then
			crisis:add_ultimate_crisis_victory_listeners()
		end
	
	end
)

-- Update the crisis settings with the player-defined customisation options.
function crisis:update_campaign_settings()

	local ssm = cm:model():shared_states_manager()

	self.settings.crisis_enabled = ssm:get_state_as_bool_value("enable_crisis_settings")

	-- Disable the crisis if the player has turned it off or it's an AI only autorun
	if self.settings.crisis_enabled == false or not cm:has_local_faction() or cm:is_benchmark_mode() then
		return
	end
	
	if self.settings.crisis_enabled == nil then
		-- Crisis settings are missing, so use a generic set of defaults
		-- These only exist to support old/broken save games that are missing data
		self.settings.crisis_enabled = true
		...
		self.settings.crisis_diplomacy_enabled = false
	else
		-- Determines whether crisis scenarios will trigger from a player hitting a certain turn (range)
		self.settings.turn_trigger = true --ssm:get_state_as_bool_value("crisis_turn_trigger")
		self.settings.turn_min = ssm:get_state_as_float_value("crisis_turn_trigger_range_min")
		self.settings.turn_max = ssm:get_state_as_float_value("crisis_turn_trigger_range_max")

		-- Determines whether crisis scenarios will trigger from a player hitting a victory condition or not
		self.settings.victory_trigger = true --ssm:get_state_as_bool_value("crisis_victory_trigger")
		
		-- Default number of turns between a warning event and the crisis fires, if one exists. Can be overridden by scenario data
		self.settings.delay = ssm:get_state_as_float_value("crisis_early_warning_delay")
		...
		self.settings.difficulty_mod = (ssm:get_state_as_float_value("crisis_difficulty_mod") or 1) / 100

		-- Tweaker that causes all crisis scenarios to fire at the same time. Why did you do this to yourself?
		self.settings.all_scenarios = ssm:get_state_as_bool_value("crisis_all_scenarios")

		-- When set to true, this allows crisis scenario factions to still do regular diplomacy and make peace
		self.settings.crisis_diplomacy_enabled = ssm:get_state_as_bool_value("crisis_diplomacy_enabled")
	end

end

-- Pre-select a scenario and warn the player(s) about it.
function crisis:choose_scenario()
	if #self.scenarios == 0 then
		out("Tried to generate an crisis, but there's no valid crisis loaded!")
		return
	else
...
-- e.g. crisis_pyramid_of_nagash.delay = 2 
function crisis:trigger_early_warning(scenario, no_bundle)
	local env = core:get_env()
	...
	local delay = scenario_env.delay or self.settings.delay or 0
	if scenario_env.early_warning_event and delay > 0 then		
		if not no_bundle then
			...
			cm:set_saved_value("crisis_scenario_data", scenario_data)
		...
		crisis:generate_crisis(scenario)
		self:reveal_regions(self.revealed_regions)
	end
end

function crisis:add_early_warning_listener(scenario_data)
	core:add_listener(
		"crisis_early_warning_listener",
		"WorldStartRound",
		function()
		...
		function()
			if self.settings.all_scenarios then
				for i = 1, #self.scenarios do
					local scenario = self.scenarios[i]
					self:generate_crisis(scenario)
				end
				cm:activate_music_trigger("ScriptedEvent_Negative", "wh3_ultimateCrisis")
				self:add_ultimate_crisis_victory_listeners()
			else
				self:generate_crisis(scenario_data.scenario)
			end
			cm:set_saved_value("crisis_scenario_data", false)
			core:remove_listener("crisis_early_warning_listener")
			self:reveal_regions(self.revealed_regions)
		end,
		true
	)
...
function crisis:add_victory_condition_listener(faction_key, incident_key, objectives)
	core:add_listener(
		"crisis_victory_condition_mission_listener_"..faction_key,
		...
		function()
			if not self.settings.all_scenarios then
				crisis:create_victory_condition(faction_key, objectives)
			end
			core:remove_listener("crisis_victory_condition_mission_listener"..faction_key)
		end,
		true
	)
end
...
function crisis:add_ultimate_crisis_victory_listeners()
	if not cm:get_saved_value("crisis_ultimate_crisis_data") then
		self.ultimate_crisis_data = {
			turn_trigger = cm:turn_number() + self.ultimate_mission_delay,
			pending_mission = true
		}
		cm:set_saved_value("crisis_ultimate_crisis_data", self.ultimate_crisis_data)
	end

	self.ultimate_crisis_data = cm:get_saved_value("crisis_ultimate_crisis_data")
	if self.ultimate_crisis_data.pending_mission then
		core:add_listener(
			"crisis_ultimate_crisis_mission_listener",
			"WorldStartRound",
			function()
			...
			function()
				...
				cm:set_saved_value("crisis_ultimate_crisis_data", self.ultimate_crisis_data)
				core:remove_listener("crisis_ultimate_crisis_mission_listener")
			end,
			true
		)
	end
	core:add_listener(
		"crisis_ultimate_crisis_victory_start_turn_listener",
		"FactionTurnStart",
		function(context)
	...
	core:add_listener(
		"crisis_ultimate_crisis_victory_end_turn_listener",
		"FactionTurnEnd",
		function(context)
			local faction = context:faction()
			return not not self.ultimate_crisis_data[faction:name()] and not self:check_at_war(faction)
...
function crisis:check_at_war(faction)
...
function crisis:generate_crisis(chosen_scenario)
	out("")
	out("****************************")
	out("Executing crisis scenario: "..chosen_scenario)
...
-- The crisis creates a new, 'ultimate' victory condition that concludes the intended campaign sandbox narrative
-- The objectives for the Ultimate Victory are determined by the individual scenario scripts
function crisis:create_victory_condition(faction_key, objectives)
...
function crisis:create_scenario_force(faction_key, region_key, army_template, unit_list, declare_war, total_armies)
	...
	if declare_war then
		...
		if region_owning_faction:is_null_interface() == false then
			crisis:declare_war(faction_key, region_owning_faction:name())
		end
	end

end

function crisis:declare_war(attacker_key, defender_key)
	...
	if defender_faction:is_null_interface() == false then
		...
		if attacker_key ~= defender_key and cm:get_faction(attacker_key):at_war_with(defender_faction) == false then
			out("Crisis: Declaring war between "..attacker_key.." and "..defender_key)
...
-- We generally always want to do this with crisis scenario factions, but it's defined separately here so it's optional (especially for modding)
function crisis:no_peace_no_confederation_only_war(hostile_faction_key)
		local human_factions = cm:get_human_factions()
		for i = 1, #human_factions do
			crisis:declare_war(hostile_faction_key, cm:get_faction(human_factions[i]):name())
		end
	if self.settings.crisis_diplomacy_enabled == false then
...
function crisis:declare_war_on_adjacent_region_owners(faction, region, subculture_to_exclude)
		...
		for i = 0, adjacent_regions:num_items() - 1 do
			...
			if region:is_abandoned() == false then
				...
				if adjacent_region_owner:is_null_interface() == false and adjacent_region_owner:subculture() ~= subculture_to_exclude then
					crisis:declare_war(faction:name(), adjacent_region_owner:name())
				end
			end
		end
	end
...
function crisis:reveal_regions(region_list)
	local human_factions = cm:get_human_factions()
	for i = 1, #human_factions do
...
-- A lot of crisis functionality needs AI factions that haven't be confederated
...
function crisis:check_faction_is_valid(faction, can_be_dead)
...
function crisis:generate_random_army(army_template, unit_list)
	local ram = random_army_manager
	local army_string = "crisis_random_army_"..army_template
...
cm:add_saving_game_callback(
	function(context)
		cm:save_named_value("crisis.triggered", crisis.triggered, context, true)
	end
)
cm:add_loading_game_callback(
	function(context)
		if not cm:is_new_game() then
			crisis.triggered = cm:load_named_value("crisis.triggered", crisis.triggered, context, true)
		end
	end
)