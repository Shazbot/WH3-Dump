episode_crisis_example = {
	episode_name = "episode_crisis_example",
	episode_set = "crisis_scenario",
	...
	{
		min_turn = 25,
		player_faction_strength = 1000000,
	},
...
episode_crisis_example.stages =
{
	-- episode_crisis_example_stage_1
	-- spawns some armies
	{
		stage_key = "episode_crisis_example_stage_1",
		...
		payloads =
		{
			...
			-- spawn_armies
			{
				...
				max_units = 10,
				times_to_apply = 3,
				unit_list = episode_crisis_example.nagash_cheap_unit_list,
				target_region_list = "location_1",
				force_owner = episode_crisis_example.armies_owner_faction_key,
			},
			-- spawn_armies 2
			{
				payload_type = "spawn_army",
		...
		get_next_stage_index = function()
			...
			if is_single_player then
				local sp_stage_index = episodes_manager:get_episode_stage_index_by_name(episode_crisis_example, "episode_crisis_example_stage_2_SP")
				return sp_stage_index
			end

			local mp_stage_index = episodes_manager:get_episode_stage_index_by_name(episode_crisis_example, "episode_crisis_example_stage_2_MP")
			return mp_stage_index
		end,
	},
	-- episode_crisis_example_stage_2_SP
	-- spawn more armies, trigger dilemma for or against nagash
	{
		stage_key = "episode_crisis_example_stage_2_SP",
		-- generic properties of the episode
		min_duration = 1,
		max_duration = 2,
		...
		payloads =
		{
			...
			-- dilemma support nagash
			{
				...
				dilemma_key = episode_crisis_example.dilemma_support_nagash,
			},
		},

		get_next_stage_index = function()
			local next_stage_index = episodes_manager:get_episode_stage_index_by_name(episode_crisis_example, "episode_crisis_example_stage_3")
			return next_stage_index
		end,
	},
	-- episode_crisis_example_stage_2_MP
	-- spawn more armies,
	...
	{
		stage_key = "episode_crisis_example_stage_2_MP",
		-- generic properties of the episode
		min_duration = 1,
		max_duration = 2,
		...
		on_started = function(self)
			...
			local nagash_support = episode_crisis_example.persistent.faction_nagash_relations
			if not is_table(nagash_support) then
				episode_crisis_example.persistent.faction_nagash_relations = {}
				nagash_support = episode_crisis_example.persistent.faction_nagash_relations
			end
	...
	-- episode_crisis_example_stage_3
	-- spawn the battle marker to fight for or against Nagash
	{
		stage_key = "episode_crisis_example_stage_3",
		-- generic properties of the episode
		min_duration = 1,
		max_duration = 2,
		payloads =
		{
			-- spawn the battle marker to fight for or against Nagash
			{
				...
				marker_key = episode_crisis_example.culmination_marker_key,
				spawn_type = "region",
				keep_after_battle = true,
				interaction_event = episode_crisis_example.culmination_marker_interaction_event,
				marker_info = "wh3_dlc25_malakai_adventures_battle_high_elves",
			},
		},

		get_next_stage_index = function()
			local is_nagash_defeated = episode_crisis_example.persistent.is_nagash_defeated
			if is_nagash_defeated then
				local next_stage_index = episodes_manager:get_episode_stage_index_by_name(episode_crisis_example, "episode_crisis_example_stage_4_nagash_stopped")
				return next_stage_index
			end

			local next_stage_index = episodes_manager:get_episode_stage_index_by_name(episode_crisis_example, "episode_crisis_example_stage_4_nagash_resurrected")
			return next_stage_index
		end,
	},
	-- episode_crisis_example_stage_4_nagash_resurrected
	...
	-- either the players supporting Nagash won the battle, or the timer ran out
	{
		...
		stage_key = "episode_crisis_example_stage_4_nagash_resurrected",
		-- generic properties of the episode
		min_duration = 1,
		max_duration = 2,
		...
		on_started = function(self)
			episode_crisis_example:resurrect_nagash_faction(episode_crisis_example.persistent.nagash_culmination_region_key)
		end,
	...
	-- the players opposing Nagash won their battle in time
	{
		...
		on_started = function(self)
			episode_crisis_example.persistent.delayed_resurrection_turn = cm:turn_number() + episode_crisis_example.resurrection_delay_turns
			core:add_listener(
				"episode_crisis_example_delayedResurrection",
				"WorldStartRound",
				true,
				function(context)
					local current_turn = cm:turn_number()
					if current_turn ~= episode_crisis_example.persistent.delayed_resurrection_turn then
						return
					end

					core:remove_listener("episode_crisis_example_delayedResurrection")
					episode_crisis_example:resurrect_nagash_faction(episode_crisis_example.persistent.nagash_original_region_key)
				end,
				true
			)
		end,
...
episode_crisis_example.persistent =
{
	-- 'current_stage_index' is MANDATORY for all active episodes
	-- current_stage_index = [number]
...
episode_crisis_example.is_available_this_game = function(self)
	-- if Nagash is not in the game (e.g. Realm of Chaos, tutorial) or is played by a human, the episode is not available
	local nagash_faction_interface = cm:get_faction(episode_crisis_example.nagash_faction_key)
	if (not nagash_faction_interface)
		or nagash_faction_interface:is_null_interface()
		or nagash_faction_interface:is_human()
	then
		return false
	end
	return true
...
-- episode_crisis_example.can_start = function(self)
...
episode_crisis_example.start_episode = function(self)
	episode_crisis_example:pick_regions()
	episodes_manager:start_stage(self, 1)
end

episode_crisis_example.pick_regions = function(self)
	if not episode_crisis_example.persistent.picked_regions then
		episode_crisis_example.persistent.picked_regions = {}
	end
...
episode_crisis_example.pick_regions_from_list = function(self, number_of_regions_to_pick, regions_list)
...
episode_crisis_example.execute_stage_payloads = function(self, stage_index)
	local stage_table = episode_crisis_example.stages[stage_index]
	...
	for i = 1, #stage_table.payloads do
		...
		local params_table = {}
		params_table.force_owner = episode_crisis_example.armies_owner_faction_key
		params_table.army_template = episode_crisis_example.army_template
		...
		if payload.payload_type == "marker" then
			episode_crisis_example.persistent.nagash_culmination_region_key = payload.region_key
		end
	end
end

episode_crisis_example.get_winning_factions = function(self)
	local did_nagash_win = not episode_crisis_example.persistent.is_nagash_defeated
...
episode_crisis_example.is_in_available_state = function(self)
	if (episodes_manager.persistent.finished_episodes[episode_crisis_example.episode_name]) then
		return false
	end
...
episode_crisis_example.resurrect_nagash_faction = function(self, region_key)
	local region_interface = cm:get_region(region_key)
	if (not region_interface) or region_interface:is_null_interface() then
		return
	end
	local x_pos, y_pos = cm:find_valid_spawn_location_for_character_from_settlement(episode_crisis_example.nagash_faction_key,
	...
	local unit_list = episode_crisis_example.nagash_ressurected_unit_list
	local generated_unit_list = payloads_executor.generate_army_payload_handler:generate_random_army(episode_crisis_example.army_template, unit_list, unit_amount)

	local resurrected_character = cm:resurrect_family_member(episode_crisis_example.persistent.nagash_family_member_cqi, episode_crisis_example.nagash_faction_key)
	...
	cm:create_force_with_existing_general(
		...
		episode_crisis_example.nagash_faction_key,
		generated_unit_list,
		region_key,
		x_pos,
		y_pos
...
core:add_listener(
	"episode_crisis_example_DilemmaChoiceMadeEvent",
	"DilemmaChoiceMadeEvent",
	function(context)
		return episode_crisis_example.dilemma_support_nagash == context:dilemma();
	end,
	function(context)
		...
		if choice == episode_crisis_example.dilemma_support_nagash_support_choice then
...
core:add_listener(
	...
	function(context)
		...
		Forced_Battle_Manager:trigger_forced_battle_with_generated_army(
			...
			episode_crisis_example.armies_owner_faction_key,
		...
		function(context)
			...
			episode_crisis_example.persistent.is_nagash_defeated = not is_attacker_supporting_nagash
			--add_marker
			local encounter_marker_object = Interactive_Marker_Manager:get_marker(episode_crisis_example.culmination_marker_key)
			if encounter_marker_object then
				encounter_marker_object:despawn_all()
			else
				out("ERROR: payloads_executor could not find marker id '"..tostring(episode_crisis_example.culmination_marker_key).."'!")
			end
		end,
		false
	)
...
cm:add_first_tick_callback(
	function()
		...
		if not episode_crisis_example:is_available_this_game() then
			return
		end
		episodes_manager:add_available_episode(episode_crisis_example)

		-- we kill the Nagash faction on the first turn, so we can resurrect him later
		local current_turn = cm:turn_number()
		if current_turn == 1 and episode_crisis_example:is_in_available_state() and not episode_crisis_example.episode_disabled then
			local nagash_faction_interface = cm:get_faction(episode_crisis_example.nagash_faction_key)
			if not nagash_faction_interface:is_dead() then
				...
				episode_crisis_example.persistent.nagash_family_member_cqi = nagash_faction_interface:faction_leader():family_member():command_queue_index()
				-- we save up the original region so we can ressurect Nagash there later
				episode_crisis_example.persistent.nagash_original_region_key = home_region_interface:name()

				cm:kill_faction(episode_crisis_example.nagash_faction_key)
			end
		end
	end
)