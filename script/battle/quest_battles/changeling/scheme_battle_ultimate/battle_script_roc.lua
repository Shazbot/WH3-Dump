...
ga_ally_tze_reinforce = gb:get_army(gb:get_player_alliance_num(), "tze_ally");
...
ga_ally_tze_reinforce:assign_to_spawn_zone_from_collection_on_message("campaign_reinforcements", tze_reinforce, false);
ga_ally_tze_reinforce:message_on_number_deployed("tzeentch_deployed", true, 1);
ga_ally_tze_reinforce:assign_to_spawn_zone_from_collection_on_message("tzeentch_deployed", tze_reinforce, false);
...
--     "tze_ally_01_in",
-- 	function()
-- 		sai_ally_horror_01 = script_ai_planner:new("ally_horror_01", ga_ai_ally_01.sunits);
-- 		sai_ally_horror_01:defend_force(ga_player_01.sunits, 15);
--     end,
-- 	true
-- )
	...
	function()
		...
		bm:repeat_callback(
			function()
				...
				if final_obj_time <= 1200 then
					sm:trigger_message("call_tze_reinforce");
				end
...
---------------------------------
-----CAMPAIGN ENEMY TZEENTCH-----
---------------------------------

local ultimate_scheme_tzeentch = tonumber(core:svr_load_string("the_changeling_tzeentch_army_reinforce"))
 
function spawn_tzeentch_campaign_army()
	if ultimate_scheme_tzeentch == 1 then
		
		ga_ally_tze_reinforce:deploy_at_random_intervals_on_message(
			"call_tze_reinforce",	 	-- message
			1, 							-- min units
			1, 							-- max units
			5000, 						-- min period
			5000, 						-- max period
		...
		ga_ally_tze_reinforce:get_army():suppress_reinforcement_adc(1);

		ga_ally_tze_reinforce:message_on_any_deployed("tze_reinforce_in");
		ga_ally_tze_reinforce:rush_on_message("tze_reinforce_in");
		gb:queue_help_on_message("tze_reinforce_in", "wh3_dlc24_tze_changeling_theatre_scheme_ultimate_hint_tze");

		ga_ally_tze_reinforce:message_on_rout_proportion("tze_reinforce_defeated", 0.99);

		gb:add_listener(
			"ritual_failed",
			function()
				if ga_ally_tze_reinforce.sunits:are_any_active_on_battlefield() == true then
					ga_ally_tze_reinforce.sunits:rout_over_time(5000);
				end;
			end,
			true
		);
		
		bm:out("Adding Tzeentch units");
	elseif ultimate_scheme_tzeentch == 0 then
		gb:message_on_time_offset("tze_reinforce_defeated", 100);
		bm:out("Not adding Tzeentch units");
	else
		bm:out("The Tzeentch reinforcement tracker is broken...");
	end
end
...
if bm:is_from_campaign() then
	...
	-- bm:out("Empire Scheme is: "..core:svr_load_string("the_changeling_empire_army_reinforce"));
	-- bm:out("Cathay Scheme is: "..core:svr_load_string("the_changeling_cathay_army_reinforce"));
	-- bm:out("Greenskins Scheme is: "..core:svr_load_string("the_changeling_greenskins_army_reinforce"));
	-- bm:out("Norsca Scheme is: "..core:svr_load_string("the_changeling_norsca_army_reinforce"));
	-- bm:out("Tzeentch Scheme is: "..core:svr_load_string("the_changeling_tzeentch_army_reinforce"));
end
